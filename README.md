...
长期运行的知识库不是一次性导入。文档会修改或删除，模型、提示词和分块配置也会变化，
而一次写入可能在原文、图、向量与同步记录全部一致之前中断。主要风险有三类：
...
第一个命令使用真实本地 Cognee 和固定输出的模型替身：脚本把同一份文档中的
`BlueQueue` 改成 `GreenQueue`，再次同步后直接查询图，并检查旧实体已经消失。第二个
命令对锁、硬删除、派生数据清理、写入、处理和状态提交等窗口注入故障，共运行 17 个
回归用例。

故障矩阵使用内存 runtime 和 CocoIndex 跟踪语义模型，证明的是建模范围内成功重试后的
收敛，不是一次真实进程崩溃的端到端测试。完整示例和测试分层见[示例说明](examples/)。
