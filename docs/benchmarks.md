...
| 送入 `add` 的文档数 | 5 | 2 |
| 没有修改却被重新处理 | 3 | 0 |
| 同步耗时中位数 | 7.0587 秒 | 9.5655 秒 |
| 最小值—最大值 | 7.0121—7.1549 秒 | 9.5555—9.5774 秒 |
| 文档身份、数量和完成状态 | 通过 | 通过 |
| 旧版本标记实体残留 | 0 | 0 |
| 应有实体缺失 | 0 | 0 |
...
```text
全量重建：7.0121, 7.0587, 7.1549
增量同步：9.5774, 9.5655, 9.5555
```

这个 6 篇文档的小场景里，增量同步没有更快。测试没有分阶段计时，因此不能把耗时差
归因到某个具体操作；它能证明的是，送入 `add` 的文档从 5 篇降到 2 篇，两种方案都通过
本文限定范围内的一致性检查，并且增量方案避免了 3 篇无变化文档的重复处理。

真实模型下，少处理文档通常意味着更少的 token、嵌入和抽取请求，但本测试没有调用真实
模型，因此不把它换算成费用或线上吞吐量。耗时也不能外推到更大的语料。

原始 JSON 记录了被测源码的 Git commit、工作树状态、依赖版本、机器信息、每次耗时和
所有一致性检查。归档报告见
...
- `verify_dataset()` 本身看不到图内容，所以 real baseline 另行读取数据集图，核对旧实体
  和应有实体；
- 这项图检查只核对场景中预先定义的新旧版本标记，不等于扫描全部图节点或向量数据。
